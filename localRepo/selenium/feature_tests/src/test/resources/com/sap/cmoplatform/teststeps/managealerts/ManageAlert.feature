...
#
  ...
  Scenario: USSC_2102_05- Validate Http Code filter drop-down values2
...
#
  ...
  Scenario Outline:USSC_2102_01- Alert r
  ule name, GTIN, UUID column values are not truncated when all columns are selected from personalization
    And select all columns to display under personalisation in manage alert app
    And the button labeled OK is clicked
